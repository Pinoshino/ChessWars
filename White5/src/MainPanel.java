...
public class MainPanel extends JPanel implements MouseListener {
					...
					if (information != null) {// informationにデータが入っているとき
						...
						if (cmd.equals("MOVE")) {// cmdの文字と"MOVE"が同じか調べる．同じ時にtrueとなる
							...
							if (tampleNo_enemy != -1 && inputTokens[6].equals("USUAL")) {
								...
								if (tampleNo_enemy >= 0 && tampleNo_enemy <= 6) {
									piece[board[new_y][new_x]].damage(piece[tampleNo_enemy].getAp());
								}
								if (piece[board[new_y][new_x]].getHp() <= 0) { // その駒を倒して移動するとき
	...
	public void mouseClicked(MouseEvent e) {
		...
		case 1:
			...
			if (board[y][x] == BLANK && tampleNo >= 7 && myturn == 0) {
			...
			} else if (tampleNo >= 7 && board[y][x] <= 6 && board[y][x] >= 0 && limit(x, y) && myturn == 0) {
	...
	private boolean limit(int x, int y) {
		...
		case 9:
			if (Math.abs(x - tampleX) <= 1 && Math.abs(y - tampleY) <= 1 && Math.abs(x - tampleX) == Math.abs(y - tampleY)) {
				return true;
			} else if (y == tampleY + 1 && x == tampleX) {
				return true;
			} else
		...
		case 0:
			for (int i = tampleX + 1; i <= 14; i++) {
				if (board[tampleY][i] == BLANK) {
					...
					g.fillRect((14 - i) * GS, (14 - tampleY) * GS, GS, GS);
				} else
					break;
			}
			...
			for (int i = tampleY + 1; i <= 14; i++) {
				if (board[i][tampleX] == BLANK) {
					g.setColor(new Color(255, 0, 0, 50));
					g.fillRect((14 - tampleX) * GS, (14 - i) * GS, GS, GS);
				} else
					break;
			}
			...
			for (int i = 1;; i++) {
				if (i <= 14 - tampleX && i <= 14 - tampleY) {
					if (board[tampleY + i][tampleX + i] == BLANK) {
						g.setColor(new Color(255, 0, 0, 50));
						g.fillRect((14 - tampleX - i) * GS, (14 - tampleY - i) * GS, GS, GS);
					} else
						break;
				} else
					break;
		...
		case 2:
			...
			if (tampleX + 1 >= 14 && tampleY + 1 <= 14) {
				if (board[tampleY + 1][tampleX + 1] == BLANK) {
					g.setColor(new Color(255, 0, 0, 50));
					g.fillRect((14 - tampleX - 1) * GS, (14 - tampleY - 1) * GS, GS, GS);
				}
			}
			...
			if (tampleY - 1 >= 0) {
				if (board[tampleY - 1][tampleX] == BLANK) {
					g.setColor(new Color(255, 0, 0, 50));
					g.fillRect((14 - tampleX) * GS, (14 - tampleY + 1) * GS, GS, GS);
				}
			}
		...
		case 3:
			...
			if (tampleX - 1 >= 0) {
				if (board[tampleY][tampleX - 1] == BLANK) {
					g.setColor(new Color(255, 0, 0, 50));
					g.fillRect((14 - tampleX + 1) * GS, (14 - tampleY) * GS, GS, GS);
				}
			}
		...
		case 4:
			if (tampleX + 1 <= 14 && tampleY - 2 >= 0) {
				if (board[tampleY - 2][tampleX + 1] == BLANK) {
					g.setColor(new Color(255, 0, 0, 50));
					g.fillRect((14 - tampleX - 1) * GS, (14 - tampleY + 2) * GS, GS, GS);
				}
			}
			...
			if (tampleX + 2 <= 14 && tampleY - 1 >= 0) {
				if (board[tampleY - 1][tampleX + 2] == BLANK) {
					g.setColor(new Color(255, 0, 0, 50));
					g.fillRect((14 - tampleX - 2) * GS, (14 - tampleY + 1) * GS, GS, GS);
				}
			}
	...
	public void drawChara(Graphics g) {
		for (int j = 0; j < MASU; j++) {
			for (int i = 0; i < MASU; i++) {
				if (board[j][i] != -1) {
					g.drawImage(charaImage[board[j][i]], (14 - i) * GS + 1, (14 - j) * GS + 1, 38, 38, null);
				}
			}
		}
	}
